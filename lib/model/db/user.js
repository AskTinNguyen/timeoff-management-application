...
module.exports = function(sequelize, DataTypes) {
  ...
  var User = sequelize.define("User", {
      ...
      classMethods: get_class_methods(sequelize),
      instanceMethods : get_instance_methods(sequelize),
    });

    // Add class methods directly to the model
    Object.assign(User, get_class_methods(sequelize));
  ...
  return {
    ...
    register_new_admin_user : function(attributes){
        ...
        return sequelize.models.User.find_by_email(attributes.email)
          ...
          .then(function(departments){
            ...
            if (!departments || departments.length === 0) {
              throw new Error('No departments found for the company');
            }

            attributes.DepartmentId = departments[0].id;

            return sequelize.models.User.create(attributes, { transaction: t });
          })
          ...
          .then(function(user){
            ...
            return Promise.all(_.map(new_departments, function(department){
              ...
              return department.save({ transaction: t });
            }));
          })
      ...
      .catch(function(error) {
        ...
        if (!error.show_to_user) {
          console.error('Failed to register new admin user:', error);
          error = new Error('Failed to register user, please contact customer service');
          error.show_to_user = true;
        }
        throw error;
      });
